<div wire:key="structural-member-manager-component" class="news-wire-component">
    <style>
    ...
    <div class="row mb-4 align-items-center g-2">
        ...
        <div class="col-md-5 text-end">
            <button class="btn btn-primary" type="button" wire:click="openCreateModal">
                <i class="ri-add-fill align-bottom"></i> Tambah Struktural Yayasan
            </button>
        </div>
    </div>
    ...
    <div class="alert alert-info mb-3">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="btn-group" role="group">
                ...
                <button type="button" class="btn btn-sm btn-soft-danger" onclick="confirmMemberBulkDelete()">
        ...
        <table class="table align-middle table-nowrap mb-0 table-striped table-sm">
            <thead class="table-light">
                <tr>
                    ...
                    <th scope="col">
                        <a href="#" wire:click.prevent="sortByColumn('jabatan')" class="text-body text-decoration-none">
                            Jabatan @if($sortBy === 'jabatan') <i class="ri-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }}-line"></i> @endif
                        </a>
                    </th>
            ...
            <tbody>
                @forelse($members as $member)
                    <tr wire:key="member-{{ $member->id }}">
                        <td>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" value="{{ $member->id }}" wire:model.live="selectedItems">
                            </div>
                        </td>
                        ...
                        <td>
                            @php
                                $jabColor = match(true) {
                                    str_contains(strtolower($member->jabatan), 'ketua') || str_contains(strtolower($member->jabatan), 'kepala') || str_contains(strtolower($member->jabatan), 'pembina') => 'text-primary fw-semibold',
                                    str_contains(strtolower($member->jabatan), 'wakil') || str_contains(strtolower($member->jabatan), 'wakasek') => 'text-info fw-semibold',
                                    str_contains(strtolower($member->jabatan), 'sekretaris') => 'text-success fw-semibold',
                                    str_contains(strtolower($member->jabatan), 'bendahara') => 'text-warning fw-semibold',
                                    str_contains(strtolower($member->jabatan), 'koordinator') || str_contains(strtolower($member->jabatan), 'kaprog') => 'text-danger fw-semibold',
                                    default => 'text-secondary'
                                };
                            @endphp
                            <span class="{{ $jabColor }}">{{ $member->jabatan }}</span>
                        </td>
                        <td>
                            <span class="badge {{ $member->gender == 'male' ? 'bg-info-subtle text-info' : 'bg-danger-subtle text-danger' }}">
                        ...
                        <td class="text-center">
                            <div class="d-flex gap-1 justify-content-center">
                                @if($member->is_active)
                                    <button type="button" class="btn btn-sm btn-warning" wire:click="toggleStatus({{ $member->id }})" title="Nonaktifkan">
                                        <i class="ri-close-circle-line"></i>
                                    </button>
                                @else
                                    <button type="button" class="btn btn-sm btn-success" wire:click="toggleStatus({{ $member->id }})" title="Aktifkan">
                                        <i class="ri-checkbox-circle-line"></i>
                                    </button>
                                @endif
                                <button type="button" class="btn btn-sm btn-info" wire:click="openInfoModal({{ $member->id }})" title="Detail">
                                    <i class="ri-eye-line"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-primary" wire:click="openEditModal({{ $member->id }})" title="Edit">
                                    <i class="ri-pencil-line"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-danger" onclick="confirmMemberDelete({{ $member->id }}, '{{ addslashes($member->name) }}')" title="Hapus">
                    ...
                    <tr>
                        <td colspan="10" class="text-center py-4">
                            <div class="text-muted">
                                <i class="ri-building-3-line" style="font-size: 48px;"></i>
                                <p class="mt-2 mb-0">Tidak ada data struktural yayasan</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    @if($members && $members->count() > 0)
        <div class="d-flex justify-content-between align-items-center mt-3">
            <div class="d-flex flex-wrap gap-2 align-items-center">
                ...
                <div class="text-muted">
                    Menampilkan {{ $members->firstItem() }} - {{ $members->lastItem() }} / {{ $members->total() }} rows
                </div>
            </div>
            <div class="pagination-wrap hstack gap-2">
                {{ $members->links('vendor.pagination.bootstrap-5-always') }}
            </div>
        </div>
    @endif
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0">
                    <div class="modal-body p-0">
                        <form wire:submit.prevent="save" autocomplete="off" id="member-form">
                            <!-- Cover Banner -->
                                ...
                                <div class="d-flex position-absolute start-0 end-0 top-0 p-3">
                                    <div class="flex-grow-1">
                                        <h5 class="modal-title text-white fw-semibold">
                                            ...
                                            {{ $editMode ? 'Edit Struktural' : 'Tambah Struktural Baru' }}
                                        </h5>
                                    </div>
                            ...
                            <div class="px-4 pb-4">
                                ...
                                <div class="text-center mb-3 mt-n4">
                                    <div class="position-relative d-inline-block">
                                        ...
                                        <!-- Upload Button -->
                                        <label for="structural-photo-trigger" class="position-absolute bottom-0 end-0 mb-n1 cursor-pointer" style="right: calc(50% - 45px); z-index: 10;" title="Ubah foto">
                                            <div class="avatar-xs">
                                        ...
                                        <input type="file" id="structural-photo-trigger" class="d-none" accept="image/png,image/jpeg,image/webp">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control @error('form.name') is-invalid @enderror"
                                               wire:model="form.name" placeholder="Nama lengkap pengurus/struktural">
                                     ...
                                     <div class="col-md-6">
                                         ...
                                         <input type="email" class="form-control @error('form.email') is-invalid @enderror"
                                                wire:model="form.email" placeholder="Email pengurus">
                                    ...
                                    <!-- Deskripsi -->
                                    <div class="col-md-12">
                                        <label class="form-label fw-semibold">Keterangan / Profil Singkat</label>
                                        <textarea class="form-control @error('form.description') is-invalid @enderror" 
                                                  wire:model="form.description" rows="3" placeholder="Profil singkat atau keterangan tambahan..."></textarea>
                                ...
                                <div class="hstack gap-2 justify-content-end mt-4 pt-3 border-top">
                                    ...
                                    <button type="submit" class="btn btn-success" wire:loading.attr="disabled">
                                        <span wire:loading.remove>{{ $editMode ? 'Simpan Perubahan' : 'Tambah Struktural' }}</span>
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width: 480px;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="ri-crop-line me-2"></i>Crop Foto Struktural</h5>
    ...
    <!-- Info Detail Modal -->
    @if($showInfoModal && $selectedMember)
        <div class="modal fade show" style="display: block;" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title text-white"><i class="ri-building-3-line me-2"></i>Detail Informasi Pengurus</h5>
                        <button type="button" class="btn-close btn-close-white" wire:click="$set('showInfoModal', false)"></button>
                    </div>
                    <div class="modal-body text-center">
                        <div class="mb-3">
                            @if($selectedMember->photo)
                                <img src="{{ asset('storage/' . $selectedMember->photo) }}" class="rounded-circle img-thumbnail" style="width: 120px; height: 120px; object-fit: cover;">
                            @else
                        ...
                        <p class="text-muted mb-3">
                            Jabatan: <strong>{{ $selectedMember->jabatan }}</strong>
                        </p>
                            ...
                            <table class="table table-sm table-bordered">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ $selectedMember->gender == 'male' ? 'Laki-laki' : 'Perempuan' }}</td>
                                    </tr>
                                    <tr>
                                        <th class="bg-light">TTL</th>
                                        <td>
                                            @if($selectedMember->birth_place || $selectedMember->birth_date)
                                                {{ $selectedMember->birth_place ?? '-' }}, {{ $selectedMember->birth_date ? $selectedMember->birth_date->format('d F Y') : '-' }}
                                            @else
                                                -
                                            @endif
                                        </td>
                        ...
                        @if($selectedMember->description)
                            <div class="text-start mt-3">
                                <h6 class="fw-semibold text-primary"><i class="ri-profile-line me-1"></i>Profil / Keterangan:</h6>
                                <div class="bg-light p-2 rounded border border-dashed" style="white-space: pre-wrap;">{{ $selectedMember->description }}</div>
                            </div>
                        @endif
                    </div>
                    <div class="modal-footer">
...
(function () {
    ...
    document.addEventListener('change', function (e) {
        if (e.target && e.target.id === 'structural-photo-trigger') {
    ...
    window.confirmMemberDelete = function (id, name) {
        const message = `Data Struktural "${name}" akan dihapus secara permanen!`;
    ...
    window.confirmMemberBulkDelete = function () {
